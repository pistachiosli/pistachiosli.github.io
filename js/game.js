;(function(){
	...
	var Game = {
		...
		init:function(i){
			...
			this.wall4 = ImageManger.getImg('wall4');
			...
			for(i = 0;i<me.mapitems.length;i++){
				...
				if(me.mapitems[i].x>-1*LEFT-100&&me.mapitems[i].x<canvas.width-LEFT){
					if(me.mapitems[i].name === 'wall'){
						...
						}else if(me.mapitems[i].type === 4){
							ctx.drawImage(me.wall4,LEFT+me.mapitems[i].x,me.mapitems[i].y,me.wallsize,me.wallsize);
						}
				...
				if(me.mapitems[i].x>Mario.movex-50&&me.mapitems[i].x<Mario.movex+50){
					...
					if(boom.state === 1){
						...
						//马里奥在下边
							...
							if(me.mapitems[i].name === 'wall'){//金币墙
								if(me.mapitems[i].type === 2){
									me.mapitems[i].type = 3;
								}else if(me.mapitems[i].type === 4){
									me.mapitems[i].type = 1;
								}
							}
							if(me.mapitems[i-1]){
		...
		masterBoomWall:function(){
			...
			for(var i = 0;i<me.masters.length;i++){
				for(var j = 12;j<me.mapitems.length;j++){
					if(me.mapitems[j].x>me.masters[i].x-20&&me.mapitems[j].x<me.masters[i].x+20){
						var boom  = me.isBoom(me.masters[i],me.mapitems[j]);
						if(boom.state === 1){
							me.masters[i].speedx *= -1;
						}
					}
				}
			}
